    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-10 mb-20">
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-xl grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          <div>
            <span className="font-heading text-3xl sm:text-4xl font-extrabold text-red-600 block">5,000+</span>
            <span className="text-xs text-slate-600 uppercase tracking-wider font-bold mt-1 block">Active Rentals</span>
          </div>
          <div>
            <span className="font-heading text-3xl sm:text-4xl font-extrabold text-red-600 block">Zero</span>
            <span className="text-xs text-slate-600 uppercase tracking-wider font-bold mt-1 block">Brokerage Options</span>
          </div>
          <div>
            <span className="font-heading text-3xl sm:text-4xl font-extrabold text-red-600 block">100%</span>
            <span className="text-xs text-slate-600 uppercase tracking-wider font-bold mt-1 block">Verified Landlords</span>
          </div>
          <div>
            <span className="font-heading text-3xl sm:text-4xl font-extrabold text-red-600 block">24-Hr</span>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 mb-16">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="font-heading text-3xl sm:text-4xl font-bold text-slate-900 mt-2">
            Explore <span className="text-red-600">Rental Properties</span>
          </h2>
          ...
          {categories.map((cat, idx) => (
            <Card3D key={idx} intensity={10} depth={20} className="h-80">
              <Link
                ...
                className="bosa-card rounded-2xl overflow-hidden p-5 flex flex-col justify-between group h-full relative shadow-md hover:border-red-600 block"
              >
                <Image
                  src={cat.image}
                  alt={cat.title}
      ...
      <section className="bg-slate-100/70 border-y border-slate-200 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b border-slate-200 pb-6">
            <div>
              <span className="text-xs font-heading font-bold uppercase tracking-widest text-red-600 block mb-1">
                Handpicked Portfolios
              </span>
              <h2 className="font-heading text-3xl sm:text-4xl font-bold text-slate-900">
                Featured <span className="text-red-600">Properties for Rent</span>
              </h2>
            </div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-heading font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
                    selectedTab === tab.key
                      ...
                      : "bg-white text-slate-700 hover:text-red-600 border border-slate-200"
                  }`}
                >
                  {tab.label}
                </button>
          ...
          <div className="text-center mt-12">
            <Link
              href="/properties"
              className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-heading font-bold text-xs uppercase tracking-wider shadow-lg hover:shadow-xl hover:opacity-95 transition-all"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-6 -right-4 sm:right-6 bg-white p-6 rounded-2xl border border-red-200 shadow-2xl text-center z-20">
              <span className="font-heading text-4xl font-extrabold text-red-600 block">50,000+</span>
          ...
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-red-50 border border-red-200 text-xs font-bold text-red-700 uppercase tracking-wider">
            ...
            <h2 className="font-heading text-3xl sm:text-5xl font-extrabold text-slate-900 leading-tight">
              India’s Most Trusted <span className="text-red-600">Rental Portal</span>
            </h2>
            ...
            <div className="space-y-3 pt-2">
              {[
                ...
                <div key={idx} className="flex items-center gap-3">
                  <div className="w-5 h-5 rounded-full bg-red-600 flex items-center justify-center flex-shrink-0 text-white font-bold text-xs shadow-sm">
                    ✓
                  </div>
            ...
            <div className="pt-4 flex items-center gap-4">
              <Link
                href="/sell"
                className="px-6 py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-heading font-bold text-xs uppercase tracking-wider shadow-md hover:shadow-lg transition-all"
              >
                Post Rental Listing
              </Link>
              <a
      ...
      <section className="bg-slate-100/70 border-t border-slate-200 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-slate-900 mt-2">
              Explore by <span className="text-red-600">Popular Rental Hub</span>
            </h2>
          </div>
            ...
            {destinations.map((loc, idx) => (
              <Card3D key={idx} intensity={10} depth={20} className="h-80">
                <Link
                  ...
                  className="bosa-card rounded-2xl overflow-hidden h-full relative flex flex-col justify-end p-5 group shadow-md hover:border-red-600 block"
                >
                  <Image
                    src={loc.image}
                    alt={loc.name}
      ...
      <section className="bg-white border-t border-b border-slate-200 py-20 text-slate-900 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href="/sell"
              className="px-8 py-3.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-heading font-bold text-xs uppercase tracking-wider shadow-lg hover:shadow-xl hover:scale-105 transition-all inline-flex items-center justify-center gap-2"
            >
