...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 shadow-md">
      ...
      {/* 1. Top Announcement Header Bar (Fresh Light Red & Crisp White) */}
      <div className="bg-red-500 py-2 px-4 text-xs text-white font-sans hidden md:block border-b border-red-400">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6">
            <span className="flex items-center gap-1.5 text-white font-medium bg-red-600/40 px-2.5 py-0.5 rounded-md">
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <Link href="/" className="flex items-center space-x-3 group">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-red-400 to-red-500 p-0.5 shadow-md flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform">
                <div className="w-full h-full bg-red-500 rounded-[10px] flex items-center justify-center">
                  <Building2 className="w-6 h-6 text-white" />
                </div>
              </div>
              <div>
                <span className="font-heading text-2xl font-extrabold tracking-tight text-slate-900 block leading-none">
                  RENTED<span className="text-red-500">.</span>
                </span>
                <span className="text-[10px] tracking-widest text-red-500 uppercase font-sans font-bold block mt-1">
                  PG • House • Flat • Shop
                </span>
              </div>
            </Link>
            ...
            <nav className="hidden lg:flex items-center space-x-8 font-heading text-sm font-bold tracking-wide">
              ...
              <Link href="/properties" className="text-slate-800 hover:text-red-500 transition-colors flex items-center gap-1">
                <span>All Rentals</span>
              </Link>

              <Link href="/properties?property_type=PG" className="text-slate-800 hover:text-red-500 transition-colors">
                PG / Hostels
              </Link>

              <Link href="/properties?property_type=Flat" className="text-slate-800 hover:text-red-500 transition-colors">
                Flats
              </Link>

              <Link href="/properties?property_type=House" className="text-slate-800 hover:text-red-500 transition-colors">
                Houses
              </Link>

              <Link href="/properties?property_type=Shop" className="text-slate-800 hover:text-red-500 transition-colors">
                Shops
              </Link>

              <Link href="/favorites" className="text-slate-800 hover:text-red-500 transition-colors flex items-center gap-1.5">
            ...
            <div className="hidden lg:flex items-center space-x-4">
              <Link
                href="/sell"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-heading font-bold text-xs uppercase tracking-wider shadow-md hover:shadow-lg transition-all"
              >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-6 pt-4 pb-8 space-y-4 font-heading shadow-xl">
          <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-red-500 font-bold">
            Home
          </Link>
          ...
          <Link href="/sell" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-red-500 font-bold">
            + Post Rental
          </Link>
          {isAuthenticated ? (
            <div className="pt-4 border-t border-slate-200 space-y-2 font-sans">
              ...
              <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="block py-2 text-red-500 font-medium">
                Sign Out
              </button>
            </div>
          ) : (
            <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="block w-full text-center py-3 rounded-xl bg-red-500 text-white font-bold text-xs uppercase tracking-wider mt-4">
              Sign In
            </Link>
          )}
        </div>
