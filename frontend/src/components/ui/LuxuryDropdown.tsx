...
export default function LuxuryDropdown({
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <ChevronDown
            className={`w-3.5 h-3.5 text-slate-400 flex-shrink-0 transition-transform duration-300 ${
              isOpen ? "transform rotate-180 text-red-400" : ""
            }`}
          />
        </div>
      </button>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 z-[9999] bg-white border border-slate-200/90 rounded-2xl shadow-[0_25px_60px_-10px_rgba(248,113,113,0.15),0_10px_20px_-5px_rgba(0,0,0,0.1)] p-2 min-w-full w-max max-w-xs animate-in fade-in zoom-in-95 duration-200">
                ...
                <div
                  ...
                  className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs cursor-pointer transition-all duration-150 ${
                    isSelected
                      ...
                      : "text-slate-900 font-semibold hover:bg-slate-100 hover:text-red-400 hover:pl-4.5"
                  }`}
                >
